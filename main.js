...
class MyPlugin extends obsidian.Plugin {
    ...
    转换背景颜色() {
        this.获取编辑器信息 ();
        if(所选文本 == ""){return};
        let _html0 = /\<span\s+style=[\"'][^\<\>]*[\"']\>[^\<\>]+\<\/span\>/gi;
        let _html1 = /^\<span\s+style=([\"'])[^\<\>]*\1\>([^\<\>]+)\<\/span\>$/i;
        let _html2 = (txt) => '\<span style=\"'+this.获取背景高亮样式(this.settings.bColor)+'\"\>'+txt+'\<\/span\>';
        ...
        if (_html3.test(所选文本)){
            ...
            if(_html1.test(所选文本)){
                ...
                所选文本 = 所选文本.replace(/\<span\s+style=[\"'][^\<\>]*[\"']\>|\<\/span\>/gi,"");
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        */
        ...
        new obsidian.Setting(containerEl)
            .setName('高亮背景样式')
            .setDesc('为所有 Html 背景高亮统一选择一种更柔和的样式，快捷键与颜色方案保持不变。')
